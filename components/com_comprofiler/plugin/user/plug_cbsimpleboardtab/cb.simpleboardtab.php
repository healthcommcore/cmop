	...
	function getField( &$field, &$user, $output, $reason, $list_compare_types ) {
		...
		if ( is_object( $user ) && $user->id ) {
			...
			if ( isset( $forum ) ) {
				...
				if ( isset( $userSettings ) ) {
					if ( $field->name == 'forumsignature' ) {
						...
						$value						=	$userSettings->ordering;
					} elseif ( $field->name == 'forumview' ) {
						$value						=	$userSettings->view;
					}
				}
			}
			...
			switch ( $output ) {
				...
				case 'htmledit':
					if ( $field->name == 'forumsignature' ) {
						$return						=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'textarea', $value, 'class="inputbox"' );
					} elseif ( $field->name == 'forumorder' ) {
						$choices					=	array();
						...
						$return						=	$this->_fieldEditToHtml( $field, $user, $reason, 'input', 'select', $value, 'class="inputbox"', $choices );
					} elseif ( $field->name == 'forumview' ) {
						$choices					=	array();
	...
	function getBeginnerMode( &$params ) {
		...
		if ( $forum && ( $forum->component == 'com_kunena' ) ) {
			...
			if ( $mode === 1 ) { //Beginner
				...
				for ( $i = 1; $i <= 11; $i++ ) {
					$fields[]						=	$this->getFieldValue( $user, (int) $this->params->get( 'sidebarBeginner' . $i, null ), 'csv' );
				}
				...
				if ( $fields[4] ) {
					if ( in_array( $fields[4], array( 'Male', 'male', 'M', 'm', 1 ) ) ) {
						$gender						=	1;
					} elseif ( in_array( $fields[4], array( 'Female', 'female', 'F', 'f', 2 ) ) ) {
						$gender						=	2;
					} else {
						$gender						=	null;
					}
