...
class getForumModel extends cbTabHandler {
	 ...
	 * @param string             $prefix
	 */
	function _unsubThread( $user, $thread, $prefix ) {
		global $_CB_database;
		
		$query												=	'DELETE FROM '	. $_CB_database->NameQuote( '#__' . $prefix . '_subscriptions' )
	 ...
	 * @param string             $prefix
	 */
	function _unsubAll( $user, $prefix ) {
		global $_CB_database;
	...
	function _unfavThread( $user, $thread, $forum ) {
		...
		$query												=	'DELETE FROM '	. $_CB_database->NameQuote( '#__' . $forum->sql . '_favorites' )
															.	"\n WHERE "		. $_CB_database->NameQuote( 'userid' )	. " = " . (int) $user->id
															.	"\n AND "		. $_CB_database->NameQuote( 'thread' )	. " = " . (int) $thread
															;
		$_CB_database->setQuery( $query );
