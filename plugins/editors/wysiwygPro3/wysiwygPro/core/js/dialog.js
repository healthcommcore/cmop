...
this.getFrameWindow(node).location.replace(loc);
}
...
var locationRegex = new RegExp('^'+this.quoteMeta(this._location).replace(/[\\]&/gi,'(\\&|\\&amp\\;)')+'#', 'gi');
url = url.replace(locationRegex, '#');
var locationRegex2 = new RegExp('^'+this.quoteMeta(this._domain + this.URL + 'core/html/')+'(iframeSecurity.htm|blank.htm)', 'gi');
...
if (style!='') {
var tagName = style.replace(/^([a-z0-9]+)[^>]*?$/gi, "$1", elm);
var tag = false;
if (range.type == 'control') {
if (range.nodes[0].tagName == tagName.toUpperCase()) {
tag = range.nodes[0];
}
} else {
...
nodeValue2 = nodeValue.replace(/#[0-9a-z]+/gi, function(x) {return dialog.hexToRGB(x);});
...
this._location = String(this.editorWindow.document.location);
