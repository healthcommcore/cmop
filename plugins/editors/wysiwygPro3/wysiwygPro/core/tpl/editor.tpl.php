...
<div id="##name##_loadMessage" class="wproLoadMessageHolder" style="width:'.htmlspecialchars($width).';height:'.htmlspecialchars($height).';display:none"><div class="wproLoadMessage" style="margin-top:'.(intval(preg_replace("/[px %]/smi", '', $height))/2 - 40).'px"><img src="'.addslashes($themeURL).'misc/loader.gif" alt="" /> '.htmlspecialchars($langEngine->get('core', 'pleaseWait')).'</div></div>' ;
...
/*<![CDATA[ */
if (!##name##_doWP) {
	document.getElementById(\'##_originalName##\').style.display=\'block\';
}
/* ]]>*/
</script>
...
$STR .= '<button type="button" title="'.$button[1].'" onclick="'.htmlspecialchars($button[2]).'" style="background-image:url(\''.htmlspecialchars($button[3]).'\');'.(($button[4]!=22) ? 'width:'.htmlspecialchars($button[4]).'px;' : '').(($button[5]!=22)?'height:'.htmlspecialchars($button[5]).'px':'').'" class="wproReady"'.((!empty($button[6])) ? ' _wp_cid="'.$button[6].'"' : '').'>&nbsp;'.((!empty($button[7])) ? (($button[7]==true) ? $button[1] : $button[7]) : '').'</button>';
...
$STR .= '<a id="##name##_designTabButton" href="javascript:WPro.##name##.showDesign();" onmousedown="WPro.tabDown(this);" class="'.(($startView == 'design')?'wproTButtonUp':'wproTButtonDown').'" title="Design"><img src="'.$asThemeURL.'buttons/tab.design.gif'.'" width="14" height="10" alt="" /> '.$langEngine->get('editor', 'design').'</a>';
endif;
if ($EDITOR->featureIsEnabled('sourceTab')) :
$STR .= '<a id="##name##_sourceTabButton" href="javascript:WPro.##name##.showSource();" onmousedown="WPro.tabDown(this);" class="'.(($startView == 'source')?'wproTButtonUp':'wproTButtonDown').'" title="Source"><img src="'.$asThemeURL.'buttons/tab.source.gif'.'" width="14" height="10" alt="" /> '.$langEngine->get('editor', 'source').'</a>';
endif;
if ($EDITOR->featureIsEnabled('previewTab')) :
$STR .= '<a id="##name##_previewTabButton" href="javascript:WPro.##name##.showPreview();" onmousedown="WPro.tabDown(this);" class="'.(($startView == 'preview')?'wproTButtonUp':'wproTButtonDown').'" title="Preview"><img src="'.$asThemeURL.'buttons/tab.preview.gif'.'" width="14" height="10" alt="" /> '.$langEngine->get('editor', 'preview').'</a>';
endif;
endif;
...
WPro.##name##.newCellInners = '.($newCellInners==NULL?'null':"'".addslashes($newCellInners)."'").';
