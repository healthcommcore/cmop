...
class spellcheckerBaseAPI {
	...
	function getLang ($lang) {
		...
		if (!preg_match('/^[a-zA-z][a-zA-z_\-]+/smi',$lang)) {
			...
			if (!$found) {
				foreach($availDicts as $v) {
					if (preg_match('/^'.quotemeta($v).'/smi', $lang) || preg_match('/^'.quotemeta($v).'/smi', $lang2)) {
						$lang = $v;
						$found = true;
						break;
					}
	...
	*/
	function markupResultsHTML ($data, $resultsArray, $ignoreArray) {
				...
				if ($in_ignore<=0) {
					foreach ($keys as $k) {
						...
						//echo '/\b('.quotemeta($k).')\b/sm';
							$pL = preg_replace('/(^|\b)('.str_replace('/', '\/', quotemeta($k)).')($|\b)/sm',  '$1<span class="wproSpellcheckerError">' . $k . '</span><input class="wproSpellcheckerInput" type="hidden" name="'.$this->htmlchars($k).'_'.$this->htmlchars($key).'" value="' . $this->htmlchars($resultsArray[$k]) . '" />$3', $pL);
						}
					}
				}
				$parts[$key] = $pL . $pR;
		...
		$results = array();
		if (preg_match_all('/<[a-z][^>]+>/si', $html, $results)) {
