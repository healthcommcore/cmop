...
?>
<input type="hidden" name="uploadID" value="<?php echo htmlspecialchars($uploadID) ?>" />
...
<div><label><input type="checkbox" name="overwrite" value="true" /> <?php echo $langEngine->get('wproCore_fileBrowser', 'overwrite') ?></label>&nbsp;&nbsp;&nbsp;&nbsp;<br /><br /></div>
<?php endif ?>
...
<div><?php echo $langEngine->get('wproCore_fileBrowser', 'resizeLargerThan') ?> <select name="resize" id="resize" onchange="sizeChanged(this.value)"><option value="<?php echo $EDITOR->maxImageWidth ?>x<?php echo $EDITOR->maxImageHeight ?>"><?php echo $EDITOR->maxImageWidth ?> x <?php echo $EDITOR->maxImageHeight ?></option><?php displaySizeOptions($defaultValues['resizeOptions']); ?><option value="custom"><?php echo $langEngine->get('wproCore_fileBrowser', 'custom') ?></option></select>
 <span style="display:none" id="hiddenResize">&nbsp;&nbsp;<input type="text" size="4" name="maxWidth" id="maxWidth" value="<?php echo $EDITOR->maxImageWidth ?>" /> x <input type="text" size="4" name="maxHeight" id="maxHeight" value="<?php echo $EDITOR->maxImageHeight ?>" /> </span> <?php echo $langEngine->get('core', 'pixels') ?></div>
<?php endif ?>
</fieldset>
<?php endif ?>
<script type="text/javascript">
