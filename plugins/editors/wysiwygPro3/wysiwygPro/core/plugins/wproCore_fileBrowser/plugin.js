...
var rl2 = p.length;
for (var j=0; j < rl2; j++) {
var name = p[j].replace(/^[\s\S]*name="([^"]*)"[\s\S]*$/gi, "$1");
...
str	+= ' '+x+'="'+WPro.htmlSpecialChars(data['object'][x])+'"';
}
str+='>';
if (data['param']) {
for(var x in data['param']) {
str+='\n<param name="'+x+'" value="'+WPro.htmlSpecialChars(data['param'][x])+'"';
if (editor.useXHTML) {
str+=' />';
} else {
str+='>';
...
str	+= ' '+x+'="'+WPro.htmlSpecialChars(data['embed'][x])+'"';
}
str+='>';
if (data['content']) {
...
var str = '<img class="wproFilePlugin'+WPro.htmlSpecialChars(className)+'" src="'+editor._domain+WPro.URL+'core/images/placeholder.gif"';
...
var embeds = html.match(/<embed [^>]*>[\s\S]*?<\/embed>/gi)
if (embeds) {
