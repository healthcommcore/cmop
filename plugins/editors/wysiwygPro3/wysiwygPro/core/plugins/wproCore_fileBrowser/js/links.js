...
if (domain == dialog._domain||domain2==dialog._domain) {
webLink = false;
}
...
form.elements['target'].value = a.getAttribute('target');
...
prefix.src = dialog._domain+dialog.themeURL+'icons/'+form.prefixFileIcon.value+'.gif';
} else {
...
if (String(links[i].getAttribute('href')).search('WPRO_TEMP_LINK_') != -1) {
a.push(links[i]);
}
}
} catch (e) {}
