...
class wproAjax {
	...
	function loadXajax() {
		if ($this->xajax == null) {
			...
			if (preg_match('/^http(|s):\/\/.*?\//smi', $url)) {
				$url = preg_replace('/^http(|s):\/\/[^\/]+/smi', '', $url);
			}
			$this->xajax->sRequestURI = preg_replace("/action=[a-z]+/smi", "action=ajax", $url);
		} 
	}
	...
	function fetchHeadContent () {
		...
		//exit(htmlspecialchars($return));
		
		$e3 = new wysiwygPro();
		if (!empty($this->editorURL)) {
			$e3->editorURL = $this->editorURL;
		}
		$e3->_doBaseConfig();
		$this->editorURL = $e3->editorURL;
		unset($e);
		...
		if ($this->usingWpAjax) {
			...
			//$return .= 'function '.$this->xajax->sWrapperPrefix.'_displayWysiwygPro(){return xajax.call("displayWysiwygPro", arguments, 1);}';
		}
