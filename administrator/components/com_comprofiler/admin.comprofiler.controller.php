...
function showUsers( $option ) {
	...
	if( $lastCBlist == 'showusers' ) {
		...
		$search					=	trim( ( $_CB_TxtIntStore->_iso != 'UTF-8' ) ? strtolower( $search ) : ( is_callable( 'mb_convert_case' ) ? mb_convert_case( $search, MB_CASE_LOWER, "UTF-8") : utf8_encode(strtolower(utf8_decode( $search ) ) ) ) );
	} else {
		$filter_type			=	0;
		$filter_status			=	0;
		$filter_logged			=	0;
